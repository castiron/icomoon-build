...
function buildStylesheet(selection, type, with_map) {
    ...
    selection.icons.forEach(function(iconEntry) {
        ...
        contentDeclarations += util.format("%s%s%s: \"\\%s\";\n", typeInfo.varPrefix, prefix, props.name, props.code.toString(16));
        sassMapItems.push(util.format("'%s': '\\%s'", props.name, props.code.toString(16)));
    });
    var output = "";
    ...
    output +=
        "@font-face {\n" +
        "\tfont-family: '" + selection.metadata.name + "';\n" +
        "\tsrc: url(font-path('" + selection.metadata.name + ".eot') + '?#iefix') format('embedded-opentype'),\n" +
        "\turl(font-path('" + selection.metadata.name + ".woff')) format('woff'),\n" +
        "\turl(font-path('" + selection.metadata.name + ".ttf')) format('truetype'),\n" +
        "\turl(font-path('" + selection.metadata.name + ".svg')) format('svg');\n" +
        "\tfont-weight: normal;\n" +
        "\tfont-style: normal;\n" +
        "}\n\n" +
        "@mixin fontIcon {\n" +
        "\tfont-family: " + selection.metadata.name + ", sans-serif;\n" +
        "}\n";
...
function buildLessSheet(selection) {
    return buildStylesheet(selection, "less", false);
}
...
function buildProject(projectFilePath, cb) {
    ...
    childProcess.execFile(binPath, childArgs, {maxBuffer: 102400*1024}, function(err, stdout, stderr) {
        ...
        try {
            ...
            result.scss = buildScssSheet(result.selection, false);
